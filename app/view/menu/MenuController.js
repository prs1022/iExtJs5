...
Ext.define("App.view.menu.MenuController", {
    ...
    init: function () {
        this.st = Ext.getCmp("menuGrid").getStore(); //通过Component获取Store
    },


    //搜索
    search: function () {
        this.getViewModel().getStore("menu").reload();
    },
    ...
    del: function (grid, rowIndex, colIndex) {
        var msg = "确认删除菜单：" + grid.getStore().getAt(rowIndex).get("reportName") + " ？";
    ...
    //保存
    menusave: function (btn) {
        var fr = this.lookupReference("menuForm").getForm();
